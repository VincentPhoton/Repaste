...
> 仓库根目录附带[可交互 HTML 原型](notch-clipboard-prototype.html)与[高保真原型](index.html)，可在浏览器中完整体验上述交互。
...
1. 下载 [`Repaste-V0.1.dmg`](Repaste-V0.1.dmg)（约 2 MB，仓库内直接下载）
2. 打开 DMG，将 **Repaste** 拖入「应用程序」文件夹
3. 首次启动时跟随 3 步引导即可开始使用
...
引导全程**不索要任何系统权限**。默认模式下选中条目即写回剪贴板，回到任意应用按 `⌘V` 粘贴；如果你在设置中开启「粘贴到正在使用的应用」（自动粘贴），才会单独引导辅助功能授权，拒绝则自动回落默认行为。
